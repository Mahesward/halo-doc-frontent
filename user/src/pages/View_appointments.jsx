...
function viewAppointments() {
  ...
  const formik = useFormik({
    initialValues: {
      ...
      userId: userData?._id,
      price: '900',
      bookingId: '6452029020a1c872d2578014',
    },
      ...
      {(() => {
        ...
        if (tabActive === 'payment') {
          return (
            <div className="px-4">
              <StripeCheckout
                name={`Book Appointment for DR.${formik.values.doctorName}`}
                amount={formik.values.price * 100}
                currency="INR"
                token={makePayment}
                stripeKey={import.meta.env.VITE_STRIPE_PUBLIC}
              >
                <button
                  type="button"
